> **Статус:** закрыт
> **Итог:** все Legends из Faction Pack’ов — 311 листов у 18 фракций, EN+RU+алиасы, бейдж в поиске — в проде с v2.6.0; список MFM пуст. В конструктор ростеров pack-листы не попадают (нет таблиц appdata) — отдельный вопрос, если владелец решит
...
- **Пометка Legends в поиске** (просьба владельца в конце захода): индекс `datasheetIndex.js`
  получил пятый слот `legends` (1/null), `useSearch` кладёт `legends: true` в результат,
  `SearchModal` рисует общий `.legends-badge` (янтарный, тот же, что в сетке юнитов и в
  ростере) рядом с именем; тест в `useSearch.test.js` (Draigo — да, Ghazghkull — нет); строка в
  чейнджлоге 2.5.1. Подменю фракции (`AppSubnav`) читает тот же индекс, но пометку не носит —
  не просили.
